   ...
   window.location.href = uri + base64(format(template, ctx))
        ...
        <div class="box">
            <div class="box-header">
                ...
                -&nbsp;&nbsp;&nbsp;
                <a href="backend.php?s2=enseignantAdd&opt=0">Nouvel Enseignant</a>
               &nbsp;&nbsp;&nbsp;   -&nbsp;&nbsp;&nbsp;
                <a href="backend.php?s2=enseignantList"><i class="fa fa-refresh "></i></a>
                &nbsp;&nbsp;&nbsp;
                <button   onclick="tableToExcel('exe1', 'Liste des enseignants')" class="btn btn-box-tool" title="Exporter en excel"><i class="fa fa-print"></i></button>
                ...
                <table id="exe1" class="table table-bordered table-striped">
                    ...
                    <tbody >
                      <?php
                       ...
                       $messagesParPage=7;
                       $nbrequery ="SELECT COUNT(*) AS total FROM enseignant where status = 1 ";
                       ...
                       if(isset($_GET['page'])) {
                        ...
                        $query = "SELECT e.* FROM enseignant e where status = 1 and  (e.nom like '%".$search."%' or e.prenom like '%".$search."%')  ORDER BY e.nom ASC LIMIT ".$premiereEntree.",".$messagesParPage;  
                        $d=mysql_query($query) or die ('erreur sql '.$query.'</br>'.mysql_error());
                         while($row  = mysql_fetch_array($d)){
                            if($row[0]){
                              ...
                              echo'<td><button name="viewEns" class="btn btn-box-tool" title="Détail" ><i class="fa fa-folder-o "></button></td>';
                              echo'<td><button name="updateEns" class="btn btn-box-tool" title="Modifier" ><i class="fa fa-pencil "></button></td>';
                              echo'<td><button name="affEns" class="btn btn-box-tool" title="Affecter" data-toggle="modal" data-target="#myModal" ><i class="fa fa-gear "></button></td>';
                ...
                echo '<p align="center">Page : '; 
                  ...
                  for($i=1; $i<=$nombreDePages; $i++) {
                    echo $i;
                       ...
                       {
                            echo '<a href="backend.php?s2=enseignantList&page='.$i.'">'.$i.'</a>';
                       }
                  }
                  echo '</p>';
                      ?>
